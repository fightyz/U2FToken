...
//				Util.arrayCopyNonAtomic(cert, (short) 0, buf, (short) 0, (short) 128);
//				apdu.setOutgoingAndSend((short) 0, (short) 128);
//				ISOException.throwIt((short)(ISO7816.SW_BYTES_REMAINING_00 + 153));
	...
	private void u2fRegister(APDU apdu, byte cla, byte p1, byte p2, short lc) {
		...
		// TODO handle the extended length case.
...
//		Util.arrayCopyNonAtomic(cert, (short) 0, buffer, (short) 0, (short) 256);
		apdu.setOutgoingAndSend((short) 0, (short) 256);

		short remaining = (short)(registerResponse.length - 259);
		if (remaining > 256) {
...
//			ISOException.throwIt((short)(ISO7816.SW_BYTES_REMAINING_00 + remaining));
	...
	private void getData(APDU apdu, byte cla, byte p1, byte p2, short lc) {
		...
		if (length == 0) { // there's still more than 256 bytes to be read
...
//			remainingLen = remainingLen > 256 ? ISO7816.SW_BYTES_REMAINING_00 : (short)(ISO7816.SW_BYTES_REMAINING_00 + remainingLen);
	...
	private void u2fAuthenticate(APDU apdu, byte cla, byte p1, byte p2, short lc) {
		...
		// Verify Key Handle
		short keyHandleLen = (short) (buffer[ISO7816.OFFSET_CDATA + 64] & 0x00ff);
